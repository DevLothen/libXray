...
func Invoke(requestJSON string) string {
	...
	case LibXrayMethodReplaceConfig:
		return invokeReplaceConfig(request.Payload)
	case LibXrayMethodGetRuntimeDiagnostics:
		return invokeGetRuntimeDiagnostics(request.Payload)
	case LibXrayMethodGetTrafficStats:
		return invokeGetTrafficStats(request.Payload)
...
func invokeReplaceConfig(payload json.RawMessage) string {
	request, err := decodePayload[ReplaceConfigRequest](payload)
	if err != nil {
		return encodeInvokeResponse(nil, err)
	}
	...
	return encodeInvokeResponse(result, nil)
}

func invokeGetRuntimeDiagnostics(payload json.RawMessage) string {
	request, err := decodePayload[GetRuntimeDiagnosticsRequest](payload)
	if err != nil {
		return encodeInvokeResponse(nil, err)
	}
	result, err := xray.GetRuntimeDiagnostics(request.BalancerTag)
	if err != nil {
		return encodeInvokeResponse(nil, err)
	}
	return encodeInvokeResponse(result, nil)
...
func invokeGetTrafficStats(payload json.RawMessage) string {
	request, err := decodePayload[GetTrafficStatsRequest](payload)
	if err != nil {
		return encodeInvokeResponse(nil, err)
	}
	result, err := xray.GetTrafficStats(request.BalancerTag, request.InboundTag)
	if err != nil {
		return encodeInvokeResponse(nil, err)
	}
	return encodeInvokeResponse(result, nil)
